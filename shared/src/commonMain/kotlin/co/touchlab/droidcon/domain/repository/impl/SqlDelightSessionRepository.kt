...
class SqlDelightSessionRepository(private val dateTimeService: DateTimeService, private val sessionQueries: SessionQueries) :
    ...
    override fun observe(id: Session.Id, conferenceId: Long): Flow<Session> =
        sessionQueries.sessionById(id.value, conferenceId).asCallbackFlow().mapToSingle(Dispatchers.Main, SessionTable::mapper)

    fun sessionById(id: Session.Id, conferenceId: Long): Session? =
        sessionQueries.sessionById(id.value, conferenceId).mapToSingleOrNull(SessionTable::mapper)

    override fun observeOrNull(id: Session.Id, conferenceId: Long): Flow<Session?> =
        sessionQueries.sessionById(id.value, conferenceId).asCallbackFlow().mapToSingleOrNull(Dispatchers.Main, SessionTable::mapper)

    override fun observeAllAttending(conferenceId: Long): Flow<List<Session>> =
        sessionQueries.attendingSessions(conferenceId).asCallbackFlow().mapToList(Dispatchers.Main, SessionTable::mapper)
    ...
    override fun allSync(conferenceId: Long): List<Session> = sessionQueries.allSessions(conferenceId).mapToList(SessionTable::mapper)

    override fun findSync(id: Session.Id, conferenceId: Long): Session? =
        sessionQueries.sessionById(id.value, conferenceId).mapToSingleOrNull(SessionTable::mapper)

    override fun observeAll(conferenceId: Long): Flow<List<Session>> =
        sessionQueries.allSessions(conferenceId).asCallbackFlow().mapToList(Dispatchers.Main, SessionTable::mapper)
